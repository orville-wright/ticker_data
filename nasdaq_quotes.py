...
class nquote:
    ...
    nasdaq_headers = { \
                    ...
                    'path': '/api/quote/XXXX/info?assetclass=stocks', \
...
# method 2
# Helper method to support getting multiple stock quote from a single connection
    def change_url_endpoint(self, symbol):
        cmi_debug = __name__+"::"+self.change_url_endpoint.__name__+".#"+str(self.yti)
        logging.info('%s - chnage url endpoint within session' % cmi_debug )
        self.symbol = symbol
        logging.info('%s - form new API URL endpoint' % cmi_debug )
        self.path = '/api/quote/' + self.symbol + '/info?assetclass=stocks'
        logging.info('%s - insert new API URL into headers' % cmi_debug )
        self.nasdaq_headers['path'] = self.path
